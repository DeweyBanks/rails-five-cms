...
//call foundation on the entire document to initialize it before any foundation functionality will work.
$(document).foundation();
...
function fFadeout(id){
    ...
    //var elementHash ="#"+id;
    //var elem = $(elementHash);
    //$(elementHash).addClass('wiggle');
    
    if (document.getElementById(id).style.display == "none") {
        //alert('hidden');
        //document.getElementById(id).style.display = "block";
        Foundation.Motion.animateIn( ("#"+id) , "fade-in");
    }else{
        Foundation.Motion.animateOut( ("#"+id) , "fade-out");
    }
...
/*
var featured_stories_container = document.querySelector('#featured-stories-container');
var featured_story_msnry = new Masonry( featured_stories_container, {
  // options
  itemSelector: '#featured-stories-container li'
});
*/
//toggle arrow in tabs 
...
/* -- this one sets and removes bg color of nav on home page - */
$window.on('scroll', function(){
  var scrollTop = $window.scrollTop();
   if( $('body').hasClass('home') ){
    $( 'header' ).toggleClass('active', scrollTop > 0);
    prev = scrollTop;
   }
});

// --- if home -- add class on mouse over  -- //
...
$( 'header' ).mouseleave(function() {
    console.log("mouse leav nav bar");
    if( $('body').hasClass('home') ){
      $( this ).removeClass('active-hover');
      $('#sub-nav-container, #about-sub').removeClass('active');
    }else if( $('body').hasClass('about-internal') ){
...
function hasScrolled(){"use strict";var l=$(window).scrollTop();Math.abs(lastScrollTop-l)<=scrollAmount||(l>lastScrollTop&&l>navbarHeight?$(".bg-slideUp, .sub-slideUp , .slideUp").css({top:-navbarHeight}):l+$(window).height()<$(document).height()&&$(".bg-slideUp, .sub-slideUp , .slideUp").css({top:0}),lastScrollTop=l)}var $window=$(window),didScroll,lastScrollTop=0,scrollAmount=10,navbarHeight=$(".slideUp").outerHeight()+1;$(window).scroll(function(l){if($('body').hasClass('about-internal')){didScroll=!0}}),setInterval(function(){didScroll&&(hasScrolled(),didScroll=!1)},250);
